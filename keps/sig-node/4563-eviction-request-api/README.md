...
1. Ability to upscale first before terminating the pods with a Deployment: [Pod Surge Example](#pod-surge-example)
...
To support the required functionality, I need a mechanism to delay pod termination/eviction until
the checkpointing orchestration has completed. Current eviction mechanisms either call the eviction
API or send a DELETE request. `pod.spec.terminationGracePeriodSeconds` is inadequate to predict the
amount of the time that checkpointing would take.
